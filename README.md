...
npx prisma studio    # GUI para inspeccionar la BDD
npx prisma migrate dev --name <descripcion>   # nueva migración tras cambiar el schema
npx prisma db seed   # re-aplicar el seed del admin
```
...
| Capa | Implementación |
|------|---------------|
| Contraseña del admin | bcrypt (no reversible) |
| Contraseñas de jueces | AES-256-GCM (reversible — el admin debe poder ver y reenviar) |
| Sesión | JWT firmado con `jose`, cookie httpOnly, sameSite lax, 7 días |
| SQL injection | Prisma usa queries parametrizadas |
| XSS | React escapa contenido por default; emails HTML se escapan a mano |
| Mensajes de login | Genéricos para no revelar si el usuario existe |
| Acciones destructivas | Texto explícito requerido (escribir "eliminar" o "borrador") |

**Notas y posibles mejoras** (no implementadas; ver discusión con el equipo):
- Sin rate limiting en login.
- Sin auditoría (no se registra quién hizo qué).
- Sin 2FA en el admin.
- Sin recovery automático de contraseña del admin (re-seed manual).
- `ENCRYPTION_KEY` y `JWT_SECRET` solo en `.env` y en Vercel: rotarlas
  invalida sesiones activas y deja las contraseñas de jueces ilegibles
  (habría que regenerarlas y reenviarlas).

---

## Deploy en Vercel

1. Importar el repo de GitHub desde https://vercel.com/new.
2. Framework: Next.js (lo detecta automáticamente). Build command default.
3. En **Environment Variables**, configurar todas las del `.env.example`.
   - Marcar **Production + Preview** para todas.
   - No marcar Development (esas son para `vercel dev`).
4. Deploy. El build incluye `prisma generate` automáticamente
   (ver `scripts.build` en `package.json`).
5. Tras el primer deploy, añadir `APP_URL` con la URL pública asignada por
   Vercel para que los emails incluyan el botón "Entrar".
6. Las migraciones de Prisma ya están aplicadas en Neon desde el primer setup;
   no hace falta volver a correrlas.

---

## Documentación adicional

- [`docs/mision-votum-contexto.md`](docs/mision-votum-contexto.md) — espec
  técnica completa del proyecto (fuente de verdad: modelo de datos, lógica
  de negocio, plan de fases).
- [`docs/mision-votum-diseno.md`](docs/mision-votum-diseno.md) — guía de
  diseño de las pantallas, flujos, estados y reglas de UI.
- [`docs/mockups/`](docs/mockups/) — los mockups originales (HTML + JSX) que
  guiaron la implementación visual.

---

## Licencia

Uso interno del Colegio Misión Montessori. No comercial.
